...
export const YouTubePage = ({ darkMode }: YouTubePageProps) => {
  type Video = { id: string; title: string; thumbnail: string; link: string; publishedAt?: string };
  ...
  useEffect(() => {
    const run = async () => {
      ...
      try {
        ...
        interface RssItem { title?: string; pubDate?: string; link?: string; guid?: string; thumbnail?: string }
        interface RssResponse { items?: RssItem[] }
